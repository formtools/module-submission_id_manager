...
// our onload handler. Assign the onsubmit handler to the two forms
Event.observe(document, 'dom:loaded', function() {

		// form 1: check the user has entered a valid submission ID
    $('change_submission_id_form').observe('submit', function(e) {
    	var rules = [];
    	...
    	if (!rsv.validate(Event.element(e), rules))
        Event.stop(e);
    });

    // form 2: check the user really wants to delete all submissions
    $('truncate_form').observe('submit', function(e) {
      if (!confirm(\"{$L["confirm_truncate_form"]}\"))
        Event.stop(e);
    });
  });
";
